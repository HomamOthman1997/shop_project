...
- Versioned Numbers API bootstrap now includes `api.capabilities` and `api.actions`, a global endpoint/scope discovery catalog for customer-built bots and partner clients. It exposes only `/api/v1/...` URLs, disabled submit reasons, and `requires_idempotency_key`; Mini App URLs stay out of this public API catalog.
- Versioned Numbers API now exposes `GET /api/v1/numbers/docs`, a self-hosted HTML API reference generated by `services/numbers/api_docs.py` from the runtime OpenAPI schema and action catalog. It is the preferred customer/partner quick reference and must not include Mini App routes.
- Versioned Numbers API now exposes `GET /api/v1/numbers/openapi.json`. The schema is generated from `services/numbers/api_schema.py` and the same action discovery catalog, includes paths/scopes/auth/idempotency requirements, and must not include Mini App routes.
- Versioned Numbers API now exposes read-only recharge/support discovery through `GET /api/v1/numbers/recharge` and `GET /api/v1/numbers/support`. These return payment methods/support categories plus explicit disabled submit actions. The discovery payloads and Mini App submit flows now share `services/numbers/customer_flows.py`; recharge proof upload and support ticket creation remain Mini App-only until Telegram review/reply dependencies are removed.
...
- Mini App price rows now expose `purchase_action` for temp/voice rows and rental options; the frontend executes purchases through that Mini App action contract and treats direct `quote_token` use as fallback compatibility only.
- Mini App bootstrap now exposes a server-owned customer surface contract under `client.tabs`, `client.actions`, and `client.features`. The frontend renders buy/orders/recharge/account/support navigation from that payload and uses backend endpoint/method values for country suggestions, account, orders, prices, purchase, recharge, support, language switching, recharge proof submission, and support ticket submission.
...
- Public API order payloads now include `api_actions`, a versioned API action discovery contract for external bots/customer integrations. It exposes only `/api/v1/numbers/...` endpoints, required scopes, disabled reasons, and `requires_idempotency_key`, and is separate from Mini App `actions`.
- Mini App order cards now consume a server-owned `actions` contract on every order payload. The backend decides which copy/refresh/second-code/replacement/alternate/recording/rental controls are enabled and exposes the Mini App endpoint/method/label key plus confirm/busy/success label keys and idempotency keys; the frontend renders and executes controls from that contract, no longer synthesizes `/mini/numbers/api/orders/{id}/...` paths or action idempotency keys, and only uses older `can_*` fields as render fallback compatibility.
- Mini App recording preview uses Telegram auth headers through the existing `headers()` helper; the old undefined `authHeaders()` reference was removed.
- Mini App recharge/support routes no longer own their core submit business logic. `customer_flows.py` owns payment-method normalization, recharge request creation/review delivery metadata, support category normalization, and support ticket creation/delivery. Mini App routes keep Telegram `initData`, multipart parsing, and UI response shaping; public API submit actions remain disabled until non-Telegram proof/reply delivery is designed.
...
- `pvapins`, `vaksms`, and `smspool` are confirmed polling-only for the current accounts. They are explicit provider-level polling exceptions in `services/numbers/provider_delivery.py`; do not turn global polling back on for all providers just to support them.
...
- Latest local validation for Numbers after public payload and voice recording service consolidation:
  ...
  - Generated `/api/v1/numbers/docs` HTML was checked locally: it includes docs/openapi links and does not contain `/mini/`.
...
- Versioned refresh endpoints should stay webhook-first for webhook-capable providers. The only approved temp SMS polling exceptions are `pvapins`, `vaksms`, and `smspool`.
