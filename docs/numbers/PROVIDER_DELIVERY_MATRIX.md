...
- Confirmed polling-only providers are explicit exceptions, not a global rollback. As of 2026-05-26, `pvapins`, `vaksms`, and `smspool` use provider polling for SMS/code retrieval because the user confirmed these accounts do not support provider webhooks.
...
- If a provider cannot be configured to send webhooks, either disable that provider for code-receiving products or explicitly mark it as a polling exception in code/docs. Do not enable global provider polling just for one upstream.
...
| `pvapins` | Required polling exception | User confirmed no webhook support; docs emphasize polling `get_sms.php`; latest rent request docs use `get_number.php` with `is_rent=1` | Generic route exists but should not be used for normal delivery | Polling-only provider; quote/purchase enabled through explicit exception | Keep `provider_sms_delivery=polling`; verify polling/no-code refund live |
| `vaksms` | Required polling exception | User confirmed no webhook support; reviewed official V0/V1/V2 docs show polling/status APIs only | Generic route exists but should not be used for normal delivery | Polling-only provider; quote/purchase enabled through explicit exception | Keep `provider_sms_delivery=polling`; verify polling/no-code refund live |
...
| `smspool` | Required polling exception | User confirmed no webhook support; reviewed official Postman collection has polling APIs only | Generic route exists but should not be used for normal delivery | Polling-only provider; quote/purchase enabled through explicit exception | Keep `provider_sms_delivery=polling`; verify polling/no-code refund live |
...
- Webhook-first refresh path with provider polling disabled by default, plus hard-coded polling exceptions for `pvapins`, `vaksms`, and `smspool`.
...
- SMSPool, VAK-SMS, and PVAPins are confirmed polling-only for current accounts and are no longer treated as webhook-quarantine providers.
...
5. Disable/quarantine unknown providers that cannot send webhook callbacks. For confirmed polling-only providers, keep the exception explicit and verify no-code refund behavior live.
