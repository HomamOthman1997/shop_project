...
@pytest.mark.asyncio
async def test_setup_ready_with_one_configured_method_and_dm_fallback(monkeypatch):
    async def fake_methods(_reseller_id):
        return [
            {"code": "good", "enabled": True, "target": "wallet-123"},
    ...
    assert status["payment_methods_ready"] is True
    assert status["payment_delivery_ready"] is True
    assert status["payment_routing_ok"] is False
    assert status["topics_enabled"] is False
    assert status["configured_methods_count"] == 1
    assert status["enabled_methods_count"] == 2
    assert status["unconfigured_enabled_methods_count"] == 1


@pytest.mark.asyncio
async def test_setup_not_ready_without_any_configured_method(monkeypatch):
    async def fake_methods(_reseller_id):
        return [
            {"code": "bad", "enabled": True, "target": "SET_NEEDS_CONFIG"},
            {"code": "off", "enabled": False, "target": "wallet-123"},
        ]
